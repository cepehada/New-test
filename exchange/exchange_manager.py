...
class ExchangeRateLimit:
    """Класс для работы с ограничениями частоты запросов к бирже"""
    ...
    async def acquire(self):
        """Получает разрешение на запрос, ожидая если необходимо"""
        async with self.lock:
            ...
            # Вычисляем эффективный лимит с учетом буфера
            effective_limit = int(self.max_requests * (1 - self.buffer / 100))

            # Проверяем, не превышен ли лимит
            if self.request_count >= effective_limit:
                # Вычисляем оставшееся время до конца периода
                remaining_time = self.period - (current_time - self.start_time)

                # Если период еще не закончился, ждем
                if remaining_time > 0:
                    logger.warning(
    ...
    async def update_limit(self, rate_limit: Dict):
        """
        Обновляет настройки ограничений
    ...
    def get_usage(self) -> Dict:
        """
        Возвращает информацию об использовании ограничений
...
class ExchangeCache:
    """Класс для кеширования данных с бирж"""
    ...
    def __init__(self, cache_ttl: Dict = None):
        ...
        Args:
            cache_ttl: Словарь с временем жизни кеша для разных типов данных
        """
        self.cache_ttl = cache_ttl or {
            'markets': 3600,        # 1 час
            'tickers': 10,          # 10 секунд
            'orderbooks': 5,        # 5 секунд
            'ohlcv': 60,            # 1 минута
            'trades': 10,           # 10 секунд
            'balance': 10,          # 10 секунд
            'orders': 10,           # 10 секунд
            'my_trades': 60         # 1 минута
        }
