...
class SymbolsUpdater:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.update_interval = self.config.get("symbols_update_interval", 3600)  # 1 час по умолчанию
        self.min_volume = self.config.get("min_daily_volume", 1000000)  # Минимальный суточный объем
        self.top_pairs_count = self.config.get("top_pairs_count", 20)  # Количество выбираемых пар
        ...
        for exchange_id in exchanges:
            try:
                ...
                for market in markets:
                    if self.config.get("include_futures", True) and market.get("future", False):
                        filtered_markets.append(market)
                    elif self.config.get("include_spot", True) and not market.get("future", False):
                        filtered_markets.append(market)
                ...
                for market in filtered_markets:
                    try:
                        ...
                        logger.warning(f"Ошибка при обработке символа {market.get('symbol')}: {str(e)}")
                
                logger.info(f"Обработано {symbols_processed} символов на бирже {exchange_id}")
                
                # Закрываем соединение с биржей
                await exchange.close()
                
            except Exception as e:
                logger.error(f"Ошибка при получении данных с биржи {exchange_id}: {str(e)}")
