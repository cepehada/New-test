...
class AdvancedOrderManager:
    """
    Менеджер для создания и управления расширенными ордерами, включая хеджирование
    и сложные ордера на разных биржах
    """
    ...
    def __init__(self, config: Dict = None):
        ...
        # Погрешность для сравнения цен в процентах
        self.price_tolerance = self.config.get("price_tolerance", 0.1)
        
        # Максимальное отклонение для хеджирования в процентах
        self.hedge_max_deviation = self.config.get("hedge_max_deviation", 0.5)
        
        # Максимальная задержка для хеджирования в секундах
        ...
        # Автоматическое отслеживание и обновление ордеров
        ...
        logger.info("Менеджер расширенных ордеров инициализирован")
    ...
    async def stop(self):
        ...
        # Сохраняем активные ордера в БД
        if self.db:
        ...
        while not self._stop_requested:
            try:
                ...
                for order in orders_to_check:
                    ...
                    try:
                        # Проверяем верхнеуровневый ордер и все суб-ордера
                        await self._check_order_status(order)
                        
                        # Если ордер с хеджированием, проверяем и хедж-ордера
                        if order.is_hedged:
    ...
    async def _check_order_status(self, order: AdvancedOrder):
        """
        Проверяет статус ордера на бирже и обновляет локальное состояние
        ...
        try:
            ...
            with ExchangeErrorHandler():
                ...
                # Преобразуем статус биржи в наш формат
                if exchange_order:
...
def _map_exchange_status(self, exchange_status: str) -> OrderStatus:
        """
        Преобразует статус ордера с биржи в наш формат
    ...
    async def create_order(
        ...
        """
        Создает и отправляет ордер на биржу
        ...
        # Конвертируем строковые типы в перечисления
            ...
            try:
                ...
                with ExchangeErrorHandler():
                    ...
                    # Трейлинг-стоп ордер
                        ...
                        callback_rate = params.get('callbackRate', 1.0)  # Значение в процентах
                ...
                # Сохраняем ордер в словаре активных ордеров
                if order.is_active():
    ...
    async def _create_complex_order(self, order: AdvancedOrder) -> AdvancedOrder:
        """
        Создает сложный ордер (брекет, OCO, TWAP, VWAP и т.д.)
    ...
    async def _execute_twap(self, order: AdvancedOrder, amount_per_slice: float, interval: float, num_slices: int):
        ...
        Args:
            ...
            interval: Интервал между ордерами в секундах
        ...
        # Сохраняем окончательный результат в БД
        if self.db:
            await self.db.save_order(order.to_dict())
    ...
    async def _hedge_order(self, order: AdvancedOrder) -> bool:
        ...
        Returns:
            bool: True, если хеджирование успешно, False в противном случае
        """
    ...
    def _map_from_ccxt_order_type(self, ccxt_type: str) -> OrderType:
        """
        Преобразует тип ордера CCXT в наш тип
