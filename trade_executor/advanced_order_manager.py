...
class OrderCreator:
    ...
    async def create_order(
        self, symbol: str, order_type: OrderType, side: OrderSide, amount: float, 
        price: Optional[float], exchange_id: str, params: Dict[str, Any] = None
    ) -> AdvancedOrder:
        """
        ...
        try:
            ...
            with ExchangeErrorHandler():
                ...
                if order_type == OrderType.MARKET:
                    ...
                    response = await self._create_special_order_type(
                        exchange, order_type, symbol, ccxt_side, amount, 
                        order.price, order.stop_price, extra_params
                    )
            ...
            self.logger.info(f"Создан ордер {order.order_id} ({order.order_type.value}) для {order.symbol}")
            return order
    ...
    async def _create_special_order_type(self, exchange, order_type, symbol, side, amount, 
                                        price, stop_price, params):
        ...
        if order_type in [OrderType.STOP_LOSS, OrderType.STOP_LIMIT]:
            ...
            if not exchange.has.get('createStopOrder', False):
                raise ValueError(f"Биржа {exchange.id} не поддерживает тейк-профит ордера")
            ...
            if not exchange.has.get('createTrailingStopOrder', False):
                raise ValueError(f"Биржа {exchange.id} не поддерживает трейлинг-стоп ордера")
            
            callback_rate = params.get('callbackRate', 1.0)
            
            return await exchange.create_order(
                symbol, 'trailing_stop', side, amount, price, 
    ...
    async def adapt_limit_price(self, symbol: str, exchange_id: str, side: OrderSide, base_price: float) -> float:
        ...
        try:
            ...
            self.logger.warning(f"Ошибка при адаптации цены для {symbol}: {str(e)}. Используется исходная цена.")
            return base_price
