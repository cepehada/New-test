...
from project.trade_executor.advanced_order_manager import AdvancedOrderManager, OrderType, OrderSide
...
class AutonomousBot:
    ...
    def __init__(self, config_path: str = None):
        ...
        self.max_risk_per_trade = self.config.get("max_risk_per_trade", 0.02)  # 2% риска
                ...
                try:
                    ...
                    for symbol in symbols:
                        ...
                        if symbol in self.last_analyzed:
                            time_since_last = datetime.now() - self.last_analyzed[symbol]
                            if time_since_last < timedelta(minutes=15):  # Не анализируем чаще, чем раз в 15 минут
                                continue
    ...
    async def _analyze_and_trade(self, symbol: str):
        ...
        try:
            ...
            if len(self.open_positions) >= self.max_open_positions:
                logger.debug(f"Достигнуто максимальное количество открытых позиций ({self.max_open_positions})")
                return
            ...
            # Выбираем тип ордера
            order_type = OrderType.MARKET if signal.get("confidence", 0) > 0.8 else OrderType.LIMIT
            ...
            if self.trading_enabled:
                ...
                logger.info(f"Создан ордер {order.order_id} для {symbol} на {exchange_id}: "
                ...
                if order.status == OrderStatus.FILLED:
                    ...
                    sl_tp = await self.sl_tp_manager.create_dynamic_sl_tp(order, market_data)
                    logger.info(f"Созданы SL/TP для ордера {order.order_id}: "
                               f"SL={sl_tp.get('sl', {}).get('order_id')}, "
                               f"TP={sl_tp.get('tp', {}).get('order_id')}")
            else:
                logger.info(f"Получен сигнал для {symbol}: {action} (стратегия: {strategy}, "
    ...
    async def _get_market_analysis(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if daily_data is not None:
                ...
                for i in range(1, len(closes)):
                    tr.append(max(
                        ...
                        abs(highs[i] - closes[i-1]),  # Текущий максимум - предыдущее закрытие
                        abs(lows[i] - closes[i-1])  # Текущий минимум - предыдущее закрытие
                    ))
            ...
            if order_book:
                ...
                bid_volume = sum(amount for price, amount in order_book.get("bids", [])[:5])
                ask_volume = sum(amount for price, amount in order_book.get("asks", [])[:5])
                
                # Средний объем
                avg_volume = (bid_volume + ask_volume) / 2 if bid_volume and ask_volume else 0
    ...
    async def _get_strategy_signal(self, strategy: str, symbol: str, market_data: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    async def _get_scalping_signal(self, symbol: str, market_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "1h" in market_data:
            ...
            if last_price > ma + 0.5 * atr:
                return {"action": "buy", "confidence": min(0.6, volatility * 10), "price": last_price}
            elif last_price < ma - 0.5 * atr:
                return {"action": "sell", "confidence": min(0.6, volatility * 10), "price": last_price}
        
        return None

    async def _get_trend_following_signal(self, symbol: str, market_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if trend == "uptrend":
            return {"action": "buy", "confidence": min(0.8, trend_strength * 10), "price": market_data.get("last_price")}
        elif trend == "downtrend":
            return {"action": "sell", "confidence": min(0.8, trend_strength * 10), "price": market_data.get("last_price")}
        
        return None

    async def _get_arbitrage_signal(self, symbol: str, market_data: Dict[str, Any]) -> Dict[str, Any]:
