...
class CapitalManager:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.min_balance_threshold = self.config.get(
        ...
        self.kelly_fraction = self.config.get(
            "kelly_fraction", 0.5
        )  # Доля от критерия Келли
        self._load_history()

    def calculate_allocation(
        self, balance: float, volatility: float, slippage: float
    ) -> float:
        """
        ...
        if current_positions_count >= self.max_positions:
            logger.warning(
                f"Достигнут лимит позиций ({self.max_positions}), новые позиции не открываются"
            )
            return 0.0
        ...
        volatility_factor = max(
            0.1, 1 - volatility
        )  # Чем выше волатильность, тем меньше сумма
    ...
    def close_position(self, position_id: str, exit_price: float, pnl: float) -> None:
        ...
        logger.info(
            f"Закрыта позиция {position_id} с P&L: {pnl:.2f} ({position['pnl_percent']:.2f}%)"
        )
    ...
    def _calculate_kelly_criterion(self, balance: float) -> float:
        ...
        profitable_trades = [
            t for t in self.position_history[-50:] if t.get("pnl_percent", 0) > 0
        ]
        if not profitable_trades:
            return self.max_risk_per_trade

        avg_win = sum(t.get("pnl_percent", 0) / 100 for t in profitable_trades) / len(
            profitable_trades
        )
        ...
        avg_loss = abs(
            sum(t.get("pnl_percent", 0) / 100 for t in losing_trades)
            / len(losing_trades)
        )

        # Формула Келли: f = (bp - q) / b
        # где b = средний выигрыш / средний проигрыш, p = вероятность выигрыша, q
        ...
        logger.info(
            f"Критерий Келли: {kelly:.4f}, используем {kelly_risk:.4f} "
    ...
    def _load_history(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Загружена история позиций: {len(self.position_history)} записей"
            )
    ...
    def get_portfolio_stats(self) -> Dict[str, Any]:
        ...
        if symbol_stats:
            ...
            stats["best_symbol"] = {
                ...
                "win_rate": (
                    best_symbol[1]["win_count"] / best_symbol[1]["count"]
                    if best_symbol[1]["count"] > 0
                    else 0
                ),
            }
            ...
            stats["worst_symbol"] = {
                ...
                "win_rate": (
                    worst_symbol[1]["win_count"] / worst_symbol[1]["count"]
                    if worst_symbol[1]["count"] > 0
                    else 0
                ),
            }
