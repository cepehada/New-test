...
from project.trade_executor.advanced_order_manager import OrderType, OrderSide, AdvancedOrder
...
class DynamicSLTPManager:
    ...
    def __init__(self, order_manager, config: Dict[str, Any] = None):
        ...
        self.trailing_activation_pct = self.config.get("trailing_activation_pct", 0.01)  # 1%
        self.trailing_step_pct = self.config.get("trailing_step_pct", 0.005)  # 0.5%
        self.atr_multiplier = self.config.get("atr_multiplier", 1.5)  # Множитель ATR для стопов
            ...
            try:
                ...
                for order_id, stop_data in stops_to_check.items():
                    try:
                        ...
                        logger.error(f"Ошибка при обновлении трейлинг-стопа для {order_id}: {str(e)}")
    ...
    async def _check_and_update_trailing_stop(self, order_id: str, stop_data: Dict[str, Any]):
        ...
        if side == OrderSide.BUY.value:  # Для длинной позиции
            ...
            if current_price > stop_data.get('highest_price', 0):
                ...
                new_stop = current_price * (1 - stop_data.get('trail_pct', self.trailing_step_pct))
                
                # Если новый стоп выше текущего, обновляем
                if new_stop > current_stop:
                    stop_data['current_stop'] = new_stop
                    logger.info(f"Обновлен трейлинг-стоп для {order_id}: {current_stop:.6f} -> {new_stop:.6f} (цена: {current_price:.6f})")
        ...
        if is_triggered:
            logger.info(f"Трейлинг-стоп для {order_id} сработал: цена {current_price:.6f}, стоп {current_stop:.6f}")
    ...
    async def _update_stop_order(self, stop_data: Dict[str, Any]):
        ...
        try:
            ...
            new_stop_order = await self._create_stop_order(
                ...
                OrderSide.SELL if stop_data.get('side') == OrderSide.BUY.value else OrderSide.BUY,
    ...
    async def _execute_stop(self, stop_data: Dict[str, Any]):
        ...
        try:
            ...
            close_order = await self.order_manager.create_order(
                ...
                side=OrderSide.SELL if stop_data.get('side') == OrderSide.BUY.value else OrderSide.BUY,
    ...
    async def create_dynamic_sl_tp(self, order: AdvancedOrder, market_data: Dict[str, Any]) -> Dict[str, AdvancedOrder]:
        """
        ...
        if order.status != OrderStatus.FILLED:
            logger.warning(f"Нельзя создать SL/TP для невыполненного ордера {order.order_id}")
            return {}
        ...
        if self.config.get("use_trailing_stop", True):
            ...
            self.trailing_stops[order.order_id] = {
                ...
                "highest_price": order.average_price if order.side == OrderSide.BUY else float('inf'),
                "lowest_price": order.average_price if order.side == OrderSide.SELL else 0,
                "parent_order_id": order.order_id,
    ...
    async def _create_stop_order(self, symbol: str, exchange_id: str, side: OrderSide, 
                                amount: float, stop_price: float, parent_order_id: str) -> Optional[AdvancedOrder]:
        """Создает стоп-ордер"""
        try:
            ...
            logger.info(f"Создан стоп-ордер {stop_order.order_id} по цене {stop_price:.6f}")
            return stop_order
    ...
    async def _calculate_sl_tp(self, order: AdvancedOrder, volatility: float, atr: Optional[float]) -> Tuple[float, float]:
        """
        ...
        if atr:
            ...
            volatility_factor = max(1.0, volatility / 0.02)  # Нормализуем относительно 2%
        ...
        logger.info(f"Рассчитаны уровни для {order.order_id}: SL={sl_level:.6f}, TP={tp_level:.6f} "
