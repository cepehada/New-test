    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if cached_result is not None:
                ...
                logger.debug(
                    "Кэш-попадание для %s (всего: %d)", key, _cache_hits.get(key, 0)
                )
            ...
            logger.debug(
                "Кэш-промах для %s (всего: %d)", key, _cache_misses.get(key, 0)
            )
            return result

        # Добавляем функцию для инвалидации кэша
        wrapper.invalidate_cache = lambda: invalidate_function_cache(
...
def save_cache_to_disk(filepath: str) -> bool:
    ...
    try:
        ...
        logger.info(
            "Кэш успешно сохранен на диск: %s (%d элементов)", filepath, len(_cache)
        )
        return True
...
def load_cache_from_disk(filepath: str) -> bool:
    ...
    try:
        ...
        if (
            ...
            or "cache" not in cache_data
            or "expiration" not in cache_data
        ):
        ...
        logger.info(
            "Кэш успешно загружен с диска: %s (%d элементов)", filepath, len(_cache)
        )
...
def cleanup_expired_items() -> int:
    ...
    keys_to_remove = [
        key
        for key, exp_time in _cache_expiration.items()
