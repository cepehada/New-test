...
async def get_exchange(exchange_id: str, config=None) -> ccxt_async.Exchange:
    ...
    if exchange_id not in _exchange_instances:
        try:
            _exchange_instances[exchange_id] = await create_exchange(
                exchange_id, config
            )
            logger.info("Добавлен экземпляр биржи %s в кэш", exchange_id)
        except Exception as e:
            logger.error(
                "Не удалось получить экземпляр биржи %s: %s", exchange_id, str(e)
            )
            return None
...
async def close_exchange(exchange_id: str) -> bool:
    ...
    if exchange_id in _exchange_instances:
        try:
            ...
            logger.error(
                "Не удалось закрыть соединение с биржей %s: %s", exchange_id, str(e)
            )
...
async def fetch_ticker(exchange_id: str, symbol: str) -> Dict:
    ...
    try:
        ...
        logger.error(
            "Ошибка при получении тикера для %s на %s: %s", symbol, exchange_id, str(e)
        )
        return {}


async def fetch_order_book(exchange_id: str, symbol: str, limit: int = 20) -> Dict:
    """
...
async def create_order(
    ...
    try:
        ...
        order = await exchange.create_order(
            symbol, type_order, side, amount, price, params
        )
        logger.info(
            ...
            side,
            amount,
            price if price else "рыночная",
        )
        return order
    except Exception as e:
        logger.error(
            "Ошибка при создании заказа на %s для %s: %s", exchange_id, symbol, str(e)
        )
        return {}


async def cancel_order(
    exchange_id: str, order_id: str, symbol: str, params: Dict = None
) -> Dict:
    """
    ...
    try:
        ...
        logger.error(
            "Ошибка при отмене заказа %s на %s: %s", order_id, exchange_id, str(e)
        )
        return {}
...
async def fetch_open_orders(
    exchange_id: str, symbol: Optional[str] = None, params: Dict = None
) -> List:
    """
    ...
    try:
        ...
        logger.error(
            "Ошибка при получении открытых заказов на %s: %s", exchange_id, str(e)
        )
        return []
...
async def fetch_closed_orders(
    ...
    try:
        ...
        logger.error(
            "Ошибка при получении закрытых заказов на %s: %s", exchange_id, str(e)
        )
        return []
...
async def fetch_my_trades(
    ...
    try:
        ...
        logger.error(
            "Ошибка при получении сделок пользователя на %s: %s", exchange_id, str(e)
        )
        return []
