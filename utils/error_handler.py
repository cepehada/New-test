...
def async_with_retry(func=None, *, retries=3, delay=1, backoff=2, exceptions=None):
    """
...
def async_log_call(func):
    ...
    async def wrapper(*args, **kwargs):
        ...
        # Логируем вызов
        logger.debug(
            "Вызов асинхронной функции %s с аргументами %s, %s", func_name, args, kwargs
        )
        ...
        # Логируем завершение
        logger.debug(
            "Асинхронная функция %s завершена с результатом: %s", func_name, result
        )
