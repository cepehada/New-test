...
from project.utils.ccxt_exchanges import (
    fetch_ticker, fetch_order_book, fetch_balance
)
...
class ArbitrageCore:
    ...
    def get_instance(cls):
        ...
        Returns:
            ArbitrageCore: Экземпляр класса
        """
    ...
    async def check_arbitrage_opportunities(
        ...
        try:
            ...
            for symbol in symbols:
                ...
                for exchange in exchanges:
                    try:
                        ...
                        if ticker and ticker.get('last') and ticker.get('bid') and ticker.get('ask'):
                            tickers[exchange] = ticker
                    except Exception as e:
                        logger.debug(
                            "Ошибка при получении тикера %s на %s: %s", 
                            symbol, exchange, str(e)
                        )
            ...
            for symbol, tickers in exchange_data.items():
                ...
                for i in range(len(exchanges_list)):
                    for j in range(i + 1, len(exchanges_list)):
                        ...
                        if (profit_margin_pct > min_profit_pct and 
                                max_volume * buy_price > min_volume_usd):
            ...
            logger.warning(
                "Ошибка при проверке арбитражных возможностей: %s",
                str(e)
            )
            return []
    ...
    async def scan_opportunities(
        self, symbols=None, exchanges=None, min_profit_pct=None
    ) -> List[ArbitrageOpportunity]:
        """
        Сканирует арбитражные возможности между указанными биржами.
        
        Args:
            symbols: Символы для сканирования
            exchanges: Биржи для сканирования
            min_profit_pct: Минимальный процент прибыли
            
        Returns:
            Список найденных возможностей
        """
        return await self.check_arbitrage_opportunities(
            symbols=symbols,
            exchanges=exchanges,
        ...
        try:
            ...
            if opportunity_age > max_age_seconds:
                logger.debug(
                    ...
                    opportunity.symbol, opportunity_age, max_age_seconds
                )
                return False
            ...
            logger.debug(
                ...
                opportunity.symbol, updated_opportunity.profit_margin_pct
            )
            return False

        except Exception as e:
            logger.warning(
                "Ошибка при проверке возможности %s: %s",
    ...
    async def check_balances(
        ...
        try:
            ...
            buy_balance = await fetch_balance(exchange_id=opportunity.buy_exchange)
            sell_balance = await fetch_balance(exchange_id=opportunity.sell_exchange)
            ...
            buy_fee = buy_amount * opportunity.buy_price * (opportunity.buy_fee_pct / 100)
            sell_fee = sell_amount * opportunity.sell_price * (opportunity.sell_fee_pct / 100)
            ...
            return {
                ...
                "expected_profit_pct": (expected_profit / buy_cost) * 100 if buy_cost > 0 else 0
            }

        except Exception as e:
            logger.warning(
                "Ошибка при проверке балансов для %s: %s", 
                opportunity.symbol, str(e)
            )
            return {}
    ...
    async def calculate_trade_sizes(
        ...
        try:
            ...
            max_trade_pct = self.config.get("arbitrage", {}).get("max_balance_pct", 25) / 100
            
            if account_balance > 0:
                max_account_trade = account_balance * max_trade_pct / opportunity.buy_price
                max_buy_amount = min(max_buy_amount, max_account_trade)
            
            # Минимальная сумма сделки
            min_trade_amount = self.config.get("arbitrage", {}).get("min_trade_amount", 10)
            ...
            if expected_profit <= 0:
                logger.debug(
                    ...
                    opportunity.symbol, expected_profit
                )
                return {}
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        if stats["opportunities_found"] > 0:
            stats["verified_ratio"] = stats["opportunities_verified"] / stats["opportunities_found"]
        else:
        ...
        stats["avg_profit"] = (
            ...
            if stats["opportunities_executed"] > 0 else 0
        )
